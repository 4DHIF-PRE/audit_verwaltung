...
expressApp.get('/questions/byAudit/:auditId', async (req, res) => {
    ...
    const result = await GetQuestionsByAuditId(auditId);
...
expressApp.post('/assignRole', async (req, res) => {
    const { firstName, lastName, roleId, auditId } = req.body;

    if (!firstName || !lastName || !roleId || !auditId) {
        return res.status(400).json({ message: "Missing required parameters." });
    }

    const result = await AssignRoleToUserForAudit(firstName, lastName, roleId, auditId);
