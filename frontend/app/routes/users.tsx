    ...
    const handleDelete = async (userId: string) => {
        try {
            const response = await fetch("http://localhost:3000/users/deleteUser", {
                ...
                }, credentials: "include", body: JSON.stringify({u_userId: userId}),
            });

            if (response.ok) {
                // Setze State, um Reload anzustoßen
                setIsModified(true);
            } else {
            ...
            <div className="container mx-auto p-4">
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full bg-white border border-gray-300">
                        ...
                        <tbody>
                        {users.map((user) => (<tr key={user.u_userId} className="dark:text-black">
                                    ...
                                    <AlertDialog>
                                        ...
                                        <AlertDialogContent>
                                            ...
                                            <AlertDialogFooter>
                                                ...
                                                {!user.u_deletedAt ? (<AlertDialogAction
                                                        onClick={() => handleDelete(user.u_userId)}
                                                    >
                                                        Delete
                                                    </AlertDialogAction>) : (<AlertDialogAction
                                                        onClick={() => restoreUser(user.u_userId)}
                                                    >
                                                        Restore
                                                    </AlertDialogAction>)}
                                            </AlertDialogFooter>
